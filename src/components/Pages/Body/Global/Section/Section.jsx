export default function Section({ header, seeAll, width, maxWidth, height, children, marginBottom, open }) {
    return (
        ...
        style={{ width, maxWidth, height, marginBottom }}>
            <div className="flex justify-between space-around w-full items-end">
                ...
                {seeAll && <a onClick={() => open()} className="text-primaryOrange font-bol text-[12px] no-underline hover:underline">{'See all'}</a>}
            </div>
            {children}
        </section>
    );
